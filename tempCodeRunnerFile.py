...
def stocks():
    ...
    summary_prompt = (
        ...
        "Start each stock with its ticker symbol in bold like this: **TICKER** \n"
        "Keep it simple and direct. No jargon. No fluff. "
        "Sentence 1: Include the exact percentage change and what specifically moved it today. Be direct, no 'likely' or 'possibly'. "
        "Sentence 2: where it sits in its 52 week range and what that means in plain English. "
        "Sentence 3: one honest observation about what this movement means for someone watching this stock. No jargon. "
        "After each stock add a line with just '---' to separate them.\n\n"
        + "\n".join(summary_lines)
    )
    ...
    return jsonify({
        ...
        "gainers": gainers,
        "losers": losers,
        "summaries": stock_chat.choices[0].message.content
    })
